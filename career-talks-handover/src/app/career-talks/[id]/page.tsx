 ...
 * File:    src/app/career-talks/[id]/page.tsx
 * Route:   /career-talks/[id]              (Details page)
 * Role:    Single-talk detail page — back-arrow title row, full-
 *          width photo banner with overlaid title + "Career Talks"
 *          chip badge, description card (date + time meta row,
 *          divider, short + long description paragraphs), middle
 *          showcase image (click-to-preview modal with magnifier
 *          hover affordance), Event Video section that plays either
 *          a YouTube iframe or native <video> based on the URL
 *          (classified by videoSourceType.ts). Skeleton states
 *          while profile or talks data is hydrating. Opts into
 *          light mode the same way the listing does.
 * COPY?:   ✅ Copy verbatim into target project at the same path.
 * Reads:   useAuth(), useTheme(), useCareerTalks(),
 *          classifyVideoUrl(), toYouTubeEmbedUrl().
 * Renders: <Navbar /> — remove if target layout already mounts one.
 * See README.md §3 (file map), §4 (integration).
 * ================================================================ */
          ...
          <section className={styles.videoSection} aria-label="Event video">
            ...
            <div className={styles.videoCard}>
              {(() => {
                ...
                // When the talk has no video URL (sourceType === "none"),
                // swap the play button for a centred fallback text.
                return (
                  <>
                    <div
