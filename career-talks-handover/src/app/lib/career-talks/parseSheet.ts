 ...
 * File:    src/app/lib/career-talks/parseSheet.ts
 * Role:    SheetJS (xlsx) parser + localStorage read/write/clear
 *          helpers. Powers the temporary upload page. Defines
 *          CAREER_TALKS_STORAGE_KEY which useCareerTalks.ts
 *          imports — keep the export name stable when refactoring.
 * COPY?:   ⚠ Copy NOW alongside the upload page. DELETE this file
 *          + the `xlsx` package dependency when the real backend
 *          API is wired into useCareerTalks.ts.
 * ================================================================ */
