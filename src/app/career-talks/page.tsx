...
/** Sort glyph — designed as a visual partner to the filter sliders
 *  SVG. Same dimensions (18×18 / 24×24 viewBox), same stroke colour
 *  (white), same stroke weight (1.5), same line caps. Renders as a
 *  classic "sort amount down" — a down arrow on the left with three
 *  horizontal lines of decreasing length on the right. */
/** Sort glyph used INSIDE the SORT CHIP pill (the small icon
 *  next to "Sort: Recently Added"). Lucide `arrow-up-down`
 *  outlined glyph with muted-gray stroke `#A0AEC0` so it reads
 *  as a label prefix, distinct from the chip's white text. */
...
/** Filled play triangle inside a white-tinted circle — overlaid on the
 *  thumbnail of every career-talk card. Static / decorative; click
 *  handling lives on the parent card. */
function PlayIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
...
export default function CareerTalksPage() {
  ...
  // `modalDirty` flips true the moment the viewer interacts with
  // anything inside the modal (toggle a checkbox, type in either
  // search box, click Clear All). It's used by the Apply button so
  // that AFTER a Clear All the button stays BLUE (not muted) —
  // signalling "you've made a change; click Apply to commit the
  // cleared state". Resets to false on every modal open, so a
  // fresh-opened modal with no existing selections still shows the
  // muted Apply look until the viewer does something.
  ...
  // ── Filter toggle helpers — multi-select per category. Adds the
  // value if absent, removes it if already present (matches the
  // checkbox affordance + /jobs behaviour). */
  ...
  // Narrow the option lists live as the viewer types into the chip.
  // Case-insensitive substring match, same as /jobs.
  const filteredEmployers = EMPLOYER_OPTIONS.filter((name) =>
    name.toLowerCase().includes(employerFilterText.trim().toLowerCase())
  );
  const filteredIndustries = INDUSTRY_OPTIONS.filter((name) =>
    name.toLowerCase().includes(industryFilterText.trim().toLowerCase())
  );
